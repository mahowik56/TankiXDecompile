	...
	public class SkinnedDecalsMesh : GenericDecalsMesh<SkinnedDecals, SkinnedDecalProjectorBase, SkinnedDecalsMesh>
	{
		...
		public void Add(Mesh a_Mesh, Transform[] a_Bones, SkinQuality a_SkinQuality, Matrix4x4 a_WorldToMeshMatrix, Matrix4x4 a_MeshToWorldMatrix)
		{
			...
			if (m_Decals.CurrentUVMode == UVMode.Project || m_Decals.CurrentUVMode == UVMode.ProjectWrapped)
			{
				...
				if (0 > activeDecalProjector.UV1RectangleIndex || activeDecalProjector.UV1RectangleIndex >= m_Decals.uvRectangles.Length)
				{
			...
			if (m_Decals.CurrentUV2Mode == UV2Mode.Project || m_Decals.CurrentUV2Mode == UV2Mode.ProjectWrapped)
			{
				...
				if (0 > activeDecalProjector.UV2RectangleIndex || activeDecalProjector.UV2RectangleIndex >= m_Decals.uv2Rectangles.Length)
				{
			...
			Add(vertices, normals, array, array3, array4, boneWeights, triangles, a_Bones, bindposes, a_SkinQuality, a_WorldToMeshMatrix, a_MeshToWorldMatrix);
			if (flag)
			{
				a_Mesh.normals = null;
			}
		...
		public void Add(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Vector2[] a_UVs, Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, Transform[] a_Bones, Matrix4x4[] a_BindPoses, SkinQuality a_SkinQuality, Matrix4x4 a_WorldToMeshMatrix, Matrix4x4 a_MeshToWorldMatrix)
		{
			try
			{
				Add(a_Vertices, a_Normals, a_Tangents, null, a_UVs, a_UV2s, a_BoneWeights, a_Triangles, a_Bones, a_BindPoses, a_SkinQuality, a_WorldToMeshMatrix, a_MeshToWorldMatrix);
			}
			catch
			{
				throw;
		...
		public void Add(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Color[] a_Colors, Vector2[] a_UVs, Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, Transform[] a_Bones, Matrix4x4[] a_BindPoses, SkinQuality a_SkinQuality, Matrix4x4 a_WorldToMeshMatrix, Matrix4x4 a_MeshToWorldMatrix)
		{
			...
			InternalAdd(a_Vertices, a_Normals, a_Tangents, a_Colors, a_UVs, a_UV2s, a_BoneWeights, a_Triangles, count2, a_BindPoses, a_SkinQuality, v, a_CullingDotProduct, worldToLocalMatrix, a_MeshToDecalsMatrix, transpose);
		...
		private void InternalAdd(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Color[] a_Colors, Vector2[] a_UVs, Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, int a_BoneIndexOffset, Matrix4x4[] a_BindPoses, SkinQuality a_SkinQuality, Vector3 a_ReversedProjectionNormal, float a_CullingDotProduct, Matrix4x4 a_WorldToDecalsMatrix, Matrix4x4 a_MeshToDecalsMatrix, Matrix4x4 a_MeshToDecalsMatrixInvertedTransposed)
		{
			...
			{
				AddSkinQuality1(a_Vertices, a_Normals, a_Tangents, a_Colors, a_UVs, a_UV2s, a_BoneWeights, a_Triangles, a_BoneIndexOffset, a_BindPoses, a_ReversedProjectionNormal, a_CullingDotProduct, a_WorldToDecalsMatrix, a_MeshToDecalsMatrix, a_MeshToDecalsMatrixInvertedTransposed);
				return;
			}
			if (a_SkinQuality == SkinQuality.Bone2 || (a_SkinQuality == SkinQuality.Auto && QualitySettings.blendWeights == BlendWeights.TwoBones))
			{
				AddSkinQuality2(a_Vertices, a_Normals, a_Tangents, a_Colors, a_UVs, a_UV2s, a_BoneWeights, a_Triangles, a_BoneIndexOffset, a_BindPoses, a_ReversedProjectionNormal, a_CullingDotProduct, a_WorldToDecalsMatrix, a_MeshToDecalsMatrix, a_MeshToDecalsMatrixInvertedTransposed);
				return;
			}
			...
			case SkinQuality.Bone4:
				AddSkinQuality4(a_Vertices, a_Normals, a_Tangents, a_Colors, a_UVs, a_UV2s, a_BoneWeights, a_Triangles, a_BoneIndexOffset, a_BindPoses, a_ReversedProjectionNormal, a_CullingDotProduct, a_WorldToDecalsMatrix, a_MeshToDecalsMatrix, a_MeshToDecalsMatrixInvertedTransposed);
				break;
			}
		}
		...
		private void AddSkinQuality1(Vector3[] a_Vertices, Vector3[] a_Normals, Vector4[] a_Tangents, Color[] a_Colors, Vector2[] a_UVs, Vector2[] a_UV2s, BoneWeight[] a_BoneWeights, int[] a_Triangles, int a_BoneIndexOffset, Matrix4x4[] a_BindPoses, Vector3 a_ReversedProjectionNormal, float a_CullingDotProduct, Matrix4x4 a_WorldToDecalsMatrix, Matrix4x4 a_MeshToDecalsMatrix, Matrix4x4 a_MeshToDecalsMatrixInvertedTransposed)
		{
			...
			for (int i = 0; i < a_Vertices.Length; i++)
			{
				...
				if (m_Decals.UseVertexColors)
				{
					...
					Color item5 = vertexColorTint * (color + base.ActiveDecalProjector.VertexColorBlending * color2);
					m_VertexColors.Add(item5);
				}
				if (m_Decals.CurrentUVMode == UVMode.TargetUV || m_Decals.CurrentUVMode == UVMode.TargetUV2)
				{
					m_UVs.Add(a_UVs[i]);
