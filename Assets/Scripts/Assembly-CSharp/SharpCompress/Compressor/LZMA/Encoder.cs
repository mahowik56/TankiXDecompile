...
namespace SharpCompress.Compressor.LZMA
{
	internal class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
	{
		...
		private class LenPriceTableEncoder : LenEncoder
		{
			...
			public new void Encode(SharpCompress.Compressor.LZMA.RangeCoder.Encoder rangeEncoder, uint symbol, uint posState)
			{
		...
		private SharpCompress.Compressor.LZMA.RangeCoder.Encoder _rangeEncoder = new SharpCompress.Compressor.LZMA.RangeCoder.Encoder();
		...
		private void Create()
		{
			...
			{
				_matchFinder.Create(_dictionarySize, 4096u, _numFastBytes, 4370u);
		...
		private uint GetOptimum(uint position, out uint backRes)
		{
			...
			uint num8 = ((lenRes < repLens[num2]) ? repLens[num2] : lenRes);
			if (num8 < 2)
			{
			...
			num9 = ((repLens[0] < 2) ? 2u : (repLens[0] + 1));
			if (num9 <= lenRes)
			{
				uint num14;
			...
			while (true)
			{
				...
				for (uint num27 = 0u; num27 < 4; num27++)
				{
					...
					if (matchLen2 >= 2)
					{
						...
						uint num32 = num6 + GetRepPrice(num27, num28, state, num5) + _isMatch[(state3.Index << 4) + num31].GetPrice0() + _literalEncoder.GetSubCoder(position + num28, _matchFinder.GetIndexByte((int)(num28 - 1 - 1))).GetPrice(true, _matchFinder.GetIndexByte((int)(num28 - 1 - (reps[num27] + 1))), _matchFinder.GetIndexByte((int)(num28 - 1)));
						state3.UpdateChar();
					...
					if (num38 == _matchDistances[num37])
					{
						if (num38 < val)
						{
							...
							if (matchLen3 >= 2)
							{
								...
								uint num42 = num40 + _isMatch[(state4.Index << 4) + num41].GetPrice0() + _literalEncoder.GetSubCoder(position + num38, _matchFinder.GetIndexByte((int)(num38 - 1 - 1))).GetPrice(true, _matchFinder.GetIndexByte((int)(num38 - (num39 + 1) - 1)), _matchFinder.GetIndexByte((int)(num38 - 1)));
								state4.UpdateChar();
		...
		public void CodeOneBlock(out long inSize, out long outSize, out bool finished)
		{
			...
			if (nowPos64 == 0)
			{
				if (_trainSize != 0)
				{
					while (_trainSize != 0 && (!_processingMode || !_matchFinder.IsDataStarved))
					{
						_matchFinder.Skip(1u);
						_trainSize--;
					}
					if (_trainSize == 0)
					{
		...
		public void Code(Stream inStream, Stream outStream, long inSize, long outSize, ICodeProgress progress)
		{
			...
			try
			{
				...
				while (true)
				{
					...
					long outSize2;
					bool finished;
					CodeOneBlock(out inSize2, out outSize2, out finished);
					if (finished)
					{
						break;
					}
		...
		public long Code(Stream inStream, bool final)
		{
			...
			try
			{
				...
				{
					long outSize;
					CodeOneBlock(out inSize, out outSize, out finished);
				}
				while (!finished);
				return inSize;
			}
		...
		public void Train(Stream trainStream)
		{
			...
			if (_trainSize != 0)
			{
				...
				while (_trainSize != 0 && !_matchFinder.IsDataStarved)
				{
					_matchFinder.Skip(1u);
					_trainSize--;
				}
