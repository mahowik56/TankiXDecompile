	...
	public class TankMovementSenderSystem : ECSSystem
	{
		...
		private void CollectCommandsAndSend(TankNode tankNode, WeaponNode weaponNode, MoveCommandType commandType)
		{
			...
			MoveCommand moveCommand = new MoveCommand
			{
				...
				TankControlVertical = GetMoveAxis(tankNode),
				TankControlHorizontal = GetTurnAxis(tankNode),
				WeaponRotationControl = GetWeaponControl(weaponNode),
				ClientTime = (int)(PreciseTime.Time * 1000.0)
			};
			SendCommand(tankNode, moveCommand);
		}
		...
		private Movement GetMovement(TankNode tankNode)
		{
			...
			return new Movement
			{
				...
				Orientation = rigidbodyTransform.rotation,
				Velocity = GetVelocity(tankNode),
