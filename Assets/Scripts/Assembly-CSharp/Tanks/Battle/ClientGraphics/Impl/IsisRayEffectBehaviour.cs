...
namespace Tanks.Battle.ClientGraphics.Impl
{
	public class IsisRayEffectBehaviour : MonoBehaviour
	{
		...
		public void UpdateTargetPosition(Transform targetTransform, Vector3 targetLocalPosition, float[] speedMultipliers, float[] pointsRandomness)
		{
			...
			Vector3 vector = targetTransform.InverseTransformPoint(base.transform.position);
			endLocalPosition = MovePoint(endLocalPosition, vector, targetLocalPosition, speed2, pointsRandomness[2]);
			...
			for (int i = 0; i < rays.Length; i++)
			{
				...
				for (int j = 0; j < num2; j++)
				{
					...
					Vector3 vector2 = Bezier.PointOnCurve(num3, bezierPoints[0], bezierPoints[1], bezierPoints[2]);
					float time = (num3 * num + textureScrollDirection * curveSpeed * Time.time) % 1f;
					Vector3 vector3 = normalized * curve.Evaluate(time);
					rays[i].SetPosition(j, vector2 + vector3);
				}
