...
namespace Tanks.Lobby.ClientGarage.Impl
{
	public class TutorialShowSystem : ECSSystem
	{
		...
		{
			TutorialContainerDialog tutorialContainerDialog = (e.dialog = dialogs.component.Get<TutorialContainerDialog>());
			...
			if (item != null)
			{
				...
				{
					tutorialContainerDialog.ConatinerImageUID = marketItem.GetComponent<ImageItemComponent>().SpriteUid;
				}
			}
			tutorialContainerDialog.ChestCount = e.ItemsCount;
			List<Animator> animators = new List<Animator>();
			if (screens.IsPresent())
			{
				animators = screens.Get().component.Animators;
			}
			tutorialContainerDialog.Show(animators);
		}
