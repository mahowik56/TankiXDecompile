...
namespace Tanks.Lobby.ClientGarage.Impl
{
	public class PremiumUiShopSystem : ECSSystem
	{
		...
		{
			foreach (Transform item in shopNode.component.PackContainer)
			{
		...
		public void FillDaysLeft(UpdateEvent e, SelfPremiumUserNode user, [JoinAll] SingleNode<PremiumToolbarUiComponent> toolbar)
		{
			if (toolbar.component.visible)
			{
				...
				string text = ((!(num > 1f)) ? string.Format(toolbar.component.hoursTextLocalizedField.Value, num2.ToString("####")) : string.Format(toolbar.component.daysTextLocalizedField.Value, num.ToString("####")));
