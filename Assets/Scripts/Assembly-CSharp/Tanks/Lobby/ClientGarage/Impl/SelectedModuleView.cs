...
namespace Tanks.Lobby.ClientGarage.Impl
{
	public class SelectedModuleView : MonoBehaviour
	{
		...
		{
			foreach (Transform item in upgrade)
			{
			...
			if (moduleItem != null)
			{
				...
				{
					GameObject gameObject = Object.Instantiate(property, upgrade);
					...
					{
						float maxParam2 = (component.CurrentParam = moduleVisualProperty.CalculateModuleEffectPropertyValue(maxLevel, maxLevel));
						component.MaxParam = maxParam2;
					}
					else
					{
						float maxParam3 = moduleVisualProperty.CalculateModuleEffectPropertyValue(maxLevel, maxLevel);
		...
		private void ShowDamageBonus(ModuleItem item, long max, long current, List<List<int>> level2PowerByTier, TankPartItem tank, TankPartItem weapon)
		{
			...
			if (num >= 0)
			{
				TankPartItem tankPartItem = ((item.TankPartModuleType != TankPartModuleType.TANK) ? weapon : tank);
