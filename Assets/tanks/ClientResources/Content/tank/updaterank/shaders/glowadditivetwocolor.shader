Shader "UpdateRankEffect/GlowAdditiveTwoColor" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1 = tmp0.xxxx * _CoreColor;
                tmp0 = tmp0.yyyy * _TintColor;
                tmp1 = tmp1 * _CoreStrength.xxxx;
                tmp0 = tmp0 * _TintStrength.xxxx + tmp1;
                tmp0 = tmp0 - _CutOutLightCore.xxxx;
                tmp0 = max(tmp0, float4(0.0, 0.0, 0.0, 0.0));
                tmp0 = min(tmp0, float4(255.0, 255.0, 255.0, 255.0));
