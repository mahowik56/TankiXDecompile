Shader "TankiOnline/Hiding/OptimizedFoliage" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                tmp1.y = unity_ObjectToWorld._m13 + unity_ObjectToWorld._m03;
                tmp1.y = tmp1.y + unity_ObjectToWorld._m23;
                tmp1.y = tmp1.y + v.color.x;
                tmp1.y = tmp1.y + v.color.y;
                tmp1.y = tmp1.y + v.color.z;
                tmp1.z = floor(tmp1.y);
                tmp1.y = tmp1.y - tmp1.z;
                tmp0.x = tmp0.x * 0.5 + tmp1.y;
                tmp0.x = tmp0.x - 0.3;
                o.texcoord7.y = saturate(tmp0.x * _HueVariation.w);
                tmp0.x = _Time.y - _HidingStartTime;
                tmp0.x = saturate(tmp0.x * _HidingSpeed);
                tmp1.y = _MaxHidingRadius - _MinHidingRadius;
                tmp0.x = tmp0.x * tmp1.y + _MinHidingRadius;
                o.texcoord7.x = saturate(tmp1.x / tmp0.x);
                tmp0.x = dot(tmp0.xyz, tmp0.xyz);
                tmp0.x = rsqrt(tmp0.x);
                tmp1.xyz = tmp0.yzw * tmp0.xxx + _WorldSpaceLightPos0.xyz;
                tmp0.xyz = tmp0.xxx * tmp0.yzw;
                tmp0.w = dot(tmp1.xyz, tmp1.xyz);
                tmp0.w = rsqrt(tmp0.w);
                tmp1.xyz = tmp0.www * tmp1.xyz;
                tmp0.w = dot(_WorldSpaceLightPos0.xyz, tmp1.xyz);
                tmp0.w = max(tmp0.w, 0.0);
                tmp0.w = dot(tmp0.xy, tmp0.xy);
                tmp0.w = tmp0.w + 0.5;
                tmp1.x = dot(tmp2.xyz, tmp0.xyz);
                tmp1.x = max(tmp1.x, 0.0);
                tmp1.y = tmp1.x * -5.55473 + -6.98316;
                tmp1.x = tmp1.x * tmp1.y;
                tmp1.x = exp(tmp1.x);
                tmp1.y = 1.0 - tmp1.x;
                tmp1.x = tmp0.w * tmp1.x + tmp1.y;
                tmp1.y = dot(tmp2.xyz, _WorldSpaceLightPos0.xyz);
                tmp1.y = max(tmp1.y, 0.0);
                tmp1.z = tmp1.y * -5.55473 + -6.98316;
                tmp1.z = tmp1.y * tmp1.z;
                tmp3.xyz = tmp1.yyy * _LightColor0.xyz;
                tmp1.y = exp(tmp1.z);
                tmp1.z = 1.0 - tmp1.y;
                tmp0.w = tmp0.w * tmp1.y + tmp1.z;
                tmp0.w = tmp1.x * tmp0.w;
                tmp1.xyz = tmp2.xyz * _TransDistortion.xxx + _WorldSpaceLightPos0.xyz;
                tmp0.x = dot(tmp0.xyz, -tmp1.xyz);
                tmp0.x = max(tmp0.x, 0.0);
                tmp0.x = log(tmp0.x);
                tmp0.x = tmp0.x * _TransFallof;
                tmp0.x = exp(tmp0.x);
                tmp1.xyz = _TransColor.xyz * _TransWeight.xxx;
                ...
                o.color1.xyz = tmp0.xyz + tmp1.xyz;
                return o;
			}
			// Keywords: DIRECTIONAL
			fout frag(v2f inp)
			{
                ...
                o.sv_target.w = min(inp.texcoord7.x, 1.0);
                return o;
			}
			ENDCG
		}
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                tmp0.x = tmp0.x - 0.3;
                o.texcoord7.y = saturate(tmp0.x * _HueVariation.w);
                tmp3.xyz = tmp1.xyz - _HidingCenter;
                tmp1.xyz = _WorldSpaceLightPos0.xyz - tmp1.xyz;
                tmp0.x = dot(tmp3.xyz, tmp3.xyz);
                tmp0.x = sqrt(tmp0.x);
                tmp1.w = _Time.y - _HidingStartTime;
                tmp1.w = saturate(tmp1.w * _HidingSpeed);
                tmp2.w = _MaxHidingRadius - _MinHidingRadius;
                tmp1.w = tmp1.w * tmp2.w + _MinHidingRadius;
                o.texcoord7.x = saturate(tmp0.x / tmp1.w);
                tmp0.x = dot(tmp0.xyz, tmp0.xyz);
                tmp0.x = rsqrt(tmp0.x);
                tmp0.xyz = tmp0.xxx * tmp0.yzw;
                tmp0.w = dot(tmp1.xyz, tmp1.xyz);
