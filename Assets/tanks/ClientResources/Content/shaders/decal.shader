	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.z = exp(tmp0.z);
                tmp1.y = _Specularity * 0.08;
                tmp2 = tex2D(_MainTex, inp.texcoord6.xy);
                tmp2.xyz = tmp2.xyz * _Color.xyz;
                o.sv_target.w = tmp2.w * _Color.w;
                tmp2.xyz = min(tmp2.xyz, float3(1.0, 1.0, 1.0));
                tmp3.xyz = -_Specularity.xxx * float3(0.08, 0.08, 0.08) + tmp2.xyz;
                tmp1.yzw = _Metallic.xxx * tmp3.xyz + tmp1.yyy;
                tmp3.xyz = float3(1.0, 1.0, 1.0) - tmp1.yzw;
                tmp1.yzw = tmp0.zzz * tmp3.xyz + tmp1.yzw;
                ...
                tmp0.xzw = tmp2.xyz * tmp0.xxx + tmp1.yzw;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1 = tex2D(_EmissionMap, inp.texcoord6.xy);
