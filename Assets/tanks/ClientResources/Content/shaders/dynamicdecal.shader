Shader "TankiOnline/DynamicDecal" {
	Properties {
		_Color ("Color", Color) = (1,1,1,1)
		_MainTex ("Color (RGB), Roughness (A)", 2D) = "white" {}
		_Roughness ("Roughness", Range(0, 1)) = 1
		[NoScaleOffset] _SurfaceMap ("Metallic(R), Occlusion(G), Emission(B)", 2D) = "black" {}
	...
	SubShader {
		LOD 100
		Tags { "PerformanceChecks" = "False" "QUEUE" = "Geometry+10" "RenderType" = "Transparent" }
		Pass {
			Name "FORWARD"
			LOD 100
			Tags { "LIGHTMODE" = "FORWARDBASE" "PerformanceChecks" = "False" "QUEUE" = "Geometry+10" "RenderType" = "Transparent" "SHADOWSUPPORT" = "true" }
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord10.w = saturate(v.color.w * 10000.0 + -_Time.y);
			...
			fout frag(v2f inp)
			{
                ...
                tmp2 = tmp2 * _Color;
                tmp2.xyz = tmp2.xyz * inp.texcoord10.xyz;
                tmp2.xyz = tmp2.xyz + tmp2.xyz;
                o.sv_target.w = tmp2.w * inp.texcoord10.w;
                ...
                tmp1.z = tmp1.z * _Roughness + 0.5;
                tmp2.w = 1.0 - tmp1.x;
                tmp1.x = tmp1.z * tmp1.x + tmp2.w;
                tmp2.w = 1.0 - tmp1.y;
                tmp1.y = tmp1.z * tmp1.y + tmp2.w;
		...
		Pass {
			Name "FORWARD_DELTA"
			LOD 100
			Tags { "LIGHTMODE" = "FORWARDADD" "PerformanceChecks" = "False" "QUEUE" = "Geometry+10" "RenderType" = "Transparent" "SHADOWSUPPORT" = "true" }
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = dot(inp.texcoord.xyz, inp.texcoord.xyz);
                tmp0.x = rsqrt(tmp0.x);
                tmp0.xyz = tmp0.xxx * inp.texcoord.xyz;
                tmp1.xyz = _WorldSpaceLightPos0.xyz - inp.texcoord1.xyz;
                ...
                tmp0.yz = max(tmp1.xw, float2(0.0, 0.0));
                ...
                tmp1.z = tmp1.y * tmp1.y;
                tmp1.y = tmp1.y * 0.5;
                tmp0.w = tmp1.z * tmp1.x + tmp0.w;
                tmp0.w = tmp0.w * tmp0.w;
                tmp0.w = tmp1.z / tmp0.w;
                tmp1.x = tmp0.z * tmp0.z;
                tmp1.z = tmp1.y * tmp1.y;
                tmp1.y = -tmp1.y * tmp1.y + 1.0;
                tmp1.y = tmp1.x * tmp1.y + tmp1.z;
                tmp1.x = dot(tmp1.xy, float2(_Roughness.x, _Metallic.x));
                tmp1.x = tmp1.x + 0.5;
                tmp1.y = tmp1.y * 4.0;
                tmp1.y = 1.0 / tmp1.y;
                tmp0.w = tmp0.w * tmp1.y;
                tmp1.y = tmp0.z * -5.55473 + -6.98316;
                tmp0.z = tmp0.z * tmp1.y;
                tmp0.z = exp(tmp0.z);
                tmp2 = tex2D(_MainTex, inp.texcoord6.xy);
                tmp2 = tmp2 * _Color;
                tmp1.yzw = tmp2.xyz * inp.texcoord10.xyz;
                o.sv_target.w = tmp2.w * inp.texcoord10.w;
                tmp1.yzw = tmp1.yzw + tmp1.yzw;
                tmp1.yzw = min(tmp1.yzw, float3(1.0, 1.0, 1.0));
                tmp2.xyz = -_Specularity.xxx * float3(0.08, 0.08, 0.08) + tmp1.yzw;
                tmp2.w = _Specularity * 0.08;
                tmp2.xyz = _Metallic.xxx * tmp2.xyz + tmp2.www;
                tmp3.xyz = float3(1.0, 1.0, 1.0) - tmp2.xyz;
                tmp2.xyz = tmp0.zzz * tmp3.xyz + tmp2.xyz;
                tmp2.xyz = tmp0.www * tmp2.xyz;
                tmp2.xyz = tmp2.xyz * _Specularity.xxx;
                tmp0.z = tmp0.x * -5.55473 + -6.98316;
                tmp0.x = tmp0.x * tmp0.z;
                tmp0.x = exp(tmp0.x);
                tmp0.z = 1.0 - tmp0.x;
                tmp0.x = tmp1.x * tmp0.x + tmp0.z;
                ...
                tmp0.xzw = tmp1.xyz * tmp0.xxx + tmp2.xyz;
		...
		Pass {
			Name "META"
			LOD 100
			Tags { "LIGHTMODE" = "META" "PerformanceChecks" = "False" "QUEUE" = "Geometry+10" "RenderType" = "Transparent" }
			ZClip Off
			ZWrite Off
			Cull Off
			Offset -5, -5
