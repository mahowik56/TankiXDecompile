...
Shader "TankiOnline/BillboardTree" {
	Properties {
	...
	SubShader {
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                tmp1 = unity_MatrixV._m02_m12_m22_m32 * tmp1.zzzz + tmp2;
                tmp1 = unity_MatrixV._m03_m13_m23_m33 * tmp3.wwww + tmp1;
                ...
                tmp0.z = floor(tmp0.y);
                o.texcoord5.y = tmp0.y - tmp0.z;
                tmp2.w = asint(tmp0.z);
                tmp0.yw = trunc(_FrameInfo.xy);
                tmp1.w = 2.0 / tmp0.y;
                tmp1.w = 1.0 / tmp1.w;
                tmp2.x = tmp0.x * tmp1.w;
                tmp2.x = floor(tmp2.x);
                tmp2.z = asint(tmp2.x);
                tmp2.yz = tmp2.zw + int2(1, 1);
                tmp2.yz = floor(tmp2.yz);
                tmp2.yz = tmp2.yz / tmp0.yw;
                tmp3.xy = tmp2.yz >= -tmp2.yz;
                tmp2.yz = frac(abs(tmp2.yz));
                tmp2.yz = tmp3.xy ? tmp2.yz : -tmp2.yz;
                tmp2.yz = tmp2.yz * tmp0.yw + v.texcoord.xy;
                o.texcoord.zw = tmp2.yz / tmp0.yw;
                tmp2.y = tmp2.x / tmp0.y;
                o.texcoord5.x = tmp1.w * tmp0.x + -tmp2.x;
                tmp0.x = tmp2.y >= -tmp2.y;
                tmp1.w = frac(abs(tmp2.y));
                tmp0.x = tmp0.x ? tmp1.w : -tmp1.w;
                tmp0.x = tmp0.x * tmp0.y + v.texcoord.x;
                o.texcoord.x = tmp0.x / tmp0.y;
                tmp0.x = tmp0.z / tmp0.w;
                tmp0.y = tmp0.x >= -tmp0.x;
                tmp0.x = frac(abs(tmp0.x));
                tmp0.x = tmp0.y ? tmp0.x : -tmp0.x;
                tmp0.x = tmp0.x * tmp0.w + v.texcoord.y;
                o.texcoord.y = tmp0.x / tmp0.w;
                tmp0.x = dot(tmp1.xyz, tmp1.xyz);
                tmp0.x = rsqrt(tmp0.x);
                tmp0.yzw = tmp0.xxx * tmp1.xyz;
                tmp1.xyz = tmp1.xyz * tmp0.xxx + float3(0.0, 0.0, 1.0);
                tmp0.xyz = tmp1.xyz * float3(0.5, 0.5, 0.5) + -tmp0.yzw;
			...
			fout frag(v2f inp)
			{
                ...
                tmp0 = tmp0 - tmp1;
                tmp0 = inp.texcoord5.xxxx * tmp0 + tmp1;
                ...
                tmp1.yzw = _HueVariation.xyz - tmp0.xyz;
                ...
                tmp1.y = tmp2.w * tmp2.w + -tmp1.x;
                tmp2 = tex2D(_FakeNormal, inp.texcoord.zw);
                ...
                tmp1.z = 1.0 - tmp1.z;
                tmp3.z = sqrt(tmp1.z);
                tmp4 = tex2D(_FakeNormal, inp.texcoord.xy);
                tmp4.xy = tmp4.wy * float2(2.0, 2.0) + float2(-1.0, -1.0);
                tmp1.z = dot(tmp4.xy, tmp4.xy);
                tmp1.z = min(tmp1.z, 1.0);
                tmp1.z = 1.0 - tmp1.z;
                tmp4.z = sqrt(tmp1.z);
                tmp3.xyz = tmp3.xyz - tmp4.xyz;
                tmp3.xyz = inp.texcoord5.yyy * tmp3.xyz + tmp4.xyz;
                tmp2.xyz = tmp2.xyz - tmp3.xyz;
                tmp2.xyz = inp.texcoord5.xxx * tmp2.xyz + tmp3.xyz;
                tmp1.z = dot(tmp2.xyz, tmp2.xyz);
                tmp1.z = rsqrt(tmp1.z);
                tmp2.xyz = tmp1.zzz * tmp2.xyz;
                tmp3.x = dot(_WorldSpaceLightPos0, unity_MatrixV._m00_m10_m20_m30);
                tmp3.y = dot(_WorldSpaceLightPos0, unity_MatrixV._m01_m11_m21_m31);
                tmp3.z = dot(_WorldSpaceLightPos0, unity_MatrixV._m02_m12_m22_m32);
                tmp1.z = dot(tmp3.xyz, tmp3.xyz);
                tmp1.z = rsqrt(tmp1.z);
                tmp3.xyz = tmp1.zzz * tmp3.xyz;
                tmp1.z = dot(tmp3.xyz, tmp2.xyz);
                tmp2.xyz = tmp2.xyz * _TransDistortion.xxx + tmp3.xyz;
                tmp1.w = saturate(tmp1.z);
                tmp1.z = tmp1.z * 0.5 + 0.5;
                tmp2.w = tmp1.w * tmp1.w;
                tmp1.w = tmp1.w * tmp2.w;
                tmp1.x = tmp1.w * tmp1.y + tmp1.x;
                tmp3.xyz = inp.texcoord2.xyz - float3(-0.0, 0.1, -0.0);
                tmp1.y = dot(tmp3.xyz, -tmp2.xyz);
                ...
                tmp2.xyz = tmp1.yyy * _TransColor.xyz;
                tmp3.xyz = tmp0.xyz * _LightColor0.xyz;
                tmp2.xyz = tmp2.xyz * tmp3.xyz;
                tmp2.xyz = tmp2.xyz * _TransWeight.xxx;
                tmp2.xyz = tmp2.xyz + tmp2.xyz;
                tmp1.yzw = _LightColor0.xyz * tmp1.zzz + tmp2.xyz;
                tmp2.xyz = inp.color1.xyz + float3(0.1, 0.1, 0.1);
                tmp1.xyz = tmp1.yzw * tmp1.xxx + tmp2.xyz;
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                tmp1.y = floor(tmp1.y);
                tmp1.z = asint(tmp1.y);
                tmp1.zw = tmp1.zw + int2(1, 1);
                tmp1.zw = floor(tmp1.zw);
                tmp1.zw = tmp1.zw / tmp0.yw;
                tmp2.xy = tmp1.zw >= -tmp1.zw;
                tmp1.zw = frac(abs(tmp1.zw));
                tmp1.zw = tmp2.xy ? tmp1.zw : -tmp1.zw;
                tmp1.zw = tmp1.zw * tmp0.yw + v.texcoord.xy;
                o.texcoord.zw = tmp1.zw / tmp0.yw;
                tmp1.z = tmp1.y / tmp0.y;
                o.texcoord5.x = tmp1.x * tmp0.x + -tmp1.y;
                tmp0.x = tmp1.z >= -tmp1.z;
                tmp1.x = frac(abs(tmp1.z));
                tmp0.x = tmp0.x ? tmp1.x : -tmp1.x;
                tmp0.x = tmp0.x * tmp0.y + v.texcoord.x;
                o.texcoord.x = tmp0.x / tmp0.y;
                tmp0.x = tmp0.z / tmp0.w;
                tmp0.y = tmp0.x >= -tmp0.x;
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = inp.texcoord5.y * tmp0.x + tmp1.w;
                tmp1 = tex2D(_MainTex, inp.texcoord.xw);
                tmp2 = tex2D(_MainTex, inp.texcoord.xy);
                tmp0.y = tmp1.w - tmp2.w;
                tmp0.y = inp.texcoord5.y * tmp0.y + tmp2.w;
                tmp0.x = tmp0.x - tmp0.y;
                tmp0.x = saturate(inp.texcoord5.x * tmp0.x + tmp0.y);
                ...
                tmp0.x = 1.0 / tmp0.x;
                tmp0.x = tmp0.x - _ZBufferParams.w;
                o.oDepth = tmp0.x / _ZBufferParams.z;
                return o;
			}
			ENDCG
		}
